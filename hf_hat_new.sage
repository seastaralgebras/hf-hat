from rasmussen_param import RasmussenParam
import pdb
...
class HeegaardDiagram():
    ...
    def __init__(self,boundary_intersections,num_pointed_regions,image_of_intersections=None):
        ...
        # lists boundary intersections with smallest index at beginning
        ...
        self.regions=range(len(self.boundary_intersections)) #number of regions
        self.regions_un=range(len(self.boundary_intersections)-num_pointed_regions) #number of unpointed regions
        ...
        while len(flatten(self.intersections_on_alphas)) < len(self.intersections):
            try:
                ...
                print([p for p in self.intersections if p not in flatten(self.intersections_on_alphas)])
                assert False
        ...
        while len(flatten(self.intersections_on_betas)) < len(self.intersections):
            try:
                ...
                print([p for p in self.intersections if p not in flatten(self.intersections_on_betas)])
                assert False
            ...
            while curr_p!=start_p or new_circle==[]:
                ...
                for (R,ind_p) in regions_with_curr_point:
                    if not found_next_point:
                        try:
                            foofoo=len(self.boundary_intersections[R])
                            if ind_p%2==0 and self.boundary_intersections[R][ind_p-1] not in new_circle:
                                found_next_point=True
                                curr_p=self.boundary_intersections[R][ind_p-1]
                            elif ind_p%2==1 and self.boundary_intersections[R][(ind_p+1)%foofoo] not in new_circle:
                                found_next_point=True
                                curr_p=self.boundary_intersections[R][(ind_p+1)%foofoo]
                        except:
                            print(ind_p)
                            print(self.boundary_intersections[R])
                            assert False
                if not found_next_point:
                    curr_p=start_p
            self.intersections_on_betas.append(new_circle)

        self.alphas=range(len(self.intersections_on_alphas))
        self.betas=range(len(self.intersections_on_betas))
        if sorted(flatten(self.intersections_on_alphas))!=list(self.intersections):
            print(sorted(flatten(self.intersections_on_alphas)))
            print(list(self.intersections))
        ...
        for p in self.intersections:
            try:
                ...
                if len(self.intersections_on_alphas[a])<=2 or len(self.intersections_on_betas[b])<=2:
                    raise Exception("Failed to orient alpha or beta circle")
                elif len(self.intersections_on_alphas[a])>2 and len(self.intersections_on_betas[b])>2:
                    try:
                        [a_pind,b_pind]=[(self.intersections_on_alphas[a]).index(p),(self.intersections_on_betas[b]).index(p)]
                    except:
                        print((self.intersections_on_alphas[a]))
                        print((self.intersections_on_alphas[a]).index[p])
                        print((self.intersections_on_betas[a]))
                        print((self.intersections_on_betas[a]).index[p])

                    (R,ind_p)=next((R,2*ind_p) for R in self.regions for ind_p in range(len(self.boundary_intersections[R])/2) if self.boundary_intersections[R][2*ind_p]==p)
                    prev_b=self.boundary_intersections[R][ind_p-1]
                    next_a=self.boundary_intersections[R][ind_p+1]
                    intersection_number=-1

                    if self.intersections_on_alphas[a][a_pind-1]==next_a:
                        intersection_number=-intersection_number
                    if self.intersections_on_betas[b][b_pind-1]!=prev_b:
                        intersection_number=-intersection_number

                    (self.intersection_incidence[p]).append(intersection_number)
            except ValueError:
                pass
        self.intersection_matrix=matrix(ZZ,len(self.alphas),len(self.betas))
        for a in self.alphas:
            for b in self.betas:
                self.intersection_matrix[a,b]=sum([n for [i,j,n] in self.intersection_incidence if (a==i and b==j)])

        # region_graph_alpha is a graph whose vertices are regions, and edges are alpha arcs separating two regions. Edges labeled by which alpha circle, and the alpha arc (as the first (according to the orientation of that alpha circle) intersection point on that alpha arc). Ditto for beta circles. (We assume each alpha circle and each beta circle has at least 3 intersection points.)
        ...
        for b in self.betas:
            for ind_p,p in enumerate(self.intersections_on_betas[b]):
                ...
                try:
                    ...
                    print(self.boundary_intersections[R])
                if len(regions_with_pq_1)!=1 or len(regions_with_pq_2)!=1:
                    print(list(self.regions))
                    print(regions_with_pq_1)
                    print(regions_with_pq_2)
        ...
        self.gr_min="Not yet initialized" # min abs grading
        self.gr_max="Not yet initialized" # max abs grading
    ...
    def p_incidence(self,D):
        return [sum([D[i] for i in self.regions_un if p in self.boundary_intersections[i]]) for p in self.intersections]

    def p_corners(self,D):
        return [len([D[i] for i in self.regions_un if p in self.boundary_intersections[i] and D[i]>0]) for p in self.intersections]
    ...
    def can_contribute(self,i,j):
        ...
        try:
            ...
            if m==1 and [a for a in D if a<0]==[]:
                ...
                return False
        except TypeError: # when no domains exist between initial and final
            return False
            ...
            for g in self.generators:
                for h in self.generators:
                    if self.can_contribute(g,h) and (not self.does_contribute(g,h)):
                        print(repr(self.generator_reps[g])+" --> "+repr(self.generator_reps[h]))
                        print(repr(self.domain_type(g,h)))
                ...
                if self.image_of_SpinC_structures[S]!=S: # Not a fixed SpinC structure. Won't generate the mapping cone complex.
                    ...
                    for gr in gradings_in_S:
                        if (gr-1) in gradings_in_S:
                            ...
                            generators_lower=[g for g in self.genSpinC[S] if self.abs_gr==gr-1]+[gr for g in self.genSpinC[S] if self.abs_gr==gr-2]
    ...
    def print_action(self):
        if self.there_is_action:
            for g in self.generators:
                print(f"{repr(g)} und the action maps to {repr(self.image_of_generators[g])}")
        else:
    ...
    def print_differentials(self):
        if not self.fully_initialized:
            self.generate_domains()
        for S in self.SpinC_structures:
            print(f"In SpinC structure {repr(S)}")
            for i in self.genSpinC[S]:
                print(f"{repr(i)} in grading {repr(self.abs_gr[i])} under the differential (possibly) maps to {repr([j for j in self.genSpinC[S] if self.can_contribute(i,j)])} with domain types (Euler char, num of boundaries): {repr([self.domain_type(i,j) for j in self.genSpinC[S] if self.can_contribute(i,j)])}")
        return None
    ...
    def pretty_print(self,output_file=None,interchanges=[],cancellations=[],relevant_SpinC='all'):
            ...
            for g in maps_to_b:
                ...
                for h in maps_from_a:
                    if arrows[(g,b)]=='red' and arrows[(a,h)]=='red':
                        ...
                        arrows[(g,h)]='green' # Coefficient still unknown, comes frm change of basis.
        ...
        for gr in self.grading_spread:
            ...
            if print_equivariantly:
                ...
                for g in fixed:
                    x_coordinates[g]=0 # there could be collision here
                non_fixed_reps=[]
                for g in non_fixed:
        ...
        for gr in self.grading_spread:
            ...
            G+=text(repr(gr)+'+C',(width/2,gr),color="blue")
    ...
    def hexagon_list(self):
        return [i for i,R in enumerate(self.boundary_intersections) if len(R)==6]

    def hex_oct_list(self):
        return [i for i,R in enumerate(self.boundary_intersections) if len(R)>5]

    def contains_region(self,D,lst):
        for i in lst:
            if D[i]!=0:
                return True
        return False

    def hexagon_regions(self):
        return [self.domains_stored[i][j][1] for i in self.generators for j in self.generators if self.can_contribute(i,j) and self.contains_region(self.domains_stored[i][j][1],self.hexagon_list())]
    
    def hex_oct_regions(self):
        return [self.domains_stored[i][j][1] for i in self.generators for j in self.generators if self.can_contribute(i,j) and self.contains_region(self.domains_stored[i][j][1],self.hex_oct_list())]
...
# trefoil = RasmussenParam(3,1,1,0)
# figure8 = RasmussenParam(5,2,1,0)
# ten161 = RasmussenParam(13,4,1,5)
...
# HFK_t37 = HeegaardDiagram(torus_3_7,2)
# HFK_t37_dc = branched_double(HFK_t37,2)

# homologything=HF_ten161.compute_homology()
# print(homologything)
# HF_ten161_dc=branched_double(HF_ten161,1)

# def regions_containing_gen(lst,H):
#     return [R for R in H.boundary_intersections for n in lst if (n in R)]
